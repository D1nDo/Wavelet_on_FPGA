...
The Discrete Wavelet Transform decomposes a discrete signal into different
frequency bands and resolution levels using a pair of digital filters:

- low-pass filter: approximation coefficients
- high-pass filter: detail coefficients

The decomposition is recursively applied to the approximation coefficients,
creating a multilevel representation of the signal.

The implemented architecture follows the Mallat algorithm:
...
The implemented wavelet family is the Daubechies-4 (Db4) wavelet introduced by
Ingrid Daubechies.
...
Daubechies wavelets are compactly supported orthogonal wavelets characterized
by:
...
The Db4 transform can be exploited for:

- noise reduction by removing insignificant high-frequency coefficients
- data compression by reducing the number of relevant coefficients
